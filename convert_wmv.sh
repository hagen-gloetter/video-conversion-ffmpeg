...
#!/bin/bash
# hg_convert_wmv_to_720p_mp4.sh - WMV zu MP4 Konverter
...
convert_video() {
    ...
    if [ $? -eq 0 ] && [ -f "$output_file" ]; then
        if mv "$input_file" "$DONE_DIR/$base_name"; then
            ...
            log_success "Erfolgreich: $base_name (${duration}s) - WMV→MP4"
            return 0
        else
...
log_info "=== WMV zu MP4 Konvertierung gestartet ==="
...
if [ $file_count -eq 0 ]; then
    log_warning "Keine WMV-Dateien gefunden!"
    exit 0
fi

log_info "Gefunden: $file_count WMV-Dateien"
log_info "========================================"
