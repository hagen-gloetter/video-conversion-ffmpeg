...
#!/bin/bash
# hg_convert_movie_to_720p.sh - Bash Version
...
get_encoder() {
    ...
    # Prüfe auf hevc_videotoolbox (Mac)
...
# Bessere Skalierung mit Aspect-Ratio Erhaltung
get_scale_filter() {
...
convert_video() {
    ...
    if [ $? -eq 0 ] && [ -f "$output_file" ]; then
        if mv "$input_file" "$DONE_DIR/$base_name"; then
            ...
            local orig_size=$(stat -f%z "$DONE_DIR/$base_name" 2>/dev/null || stat -c%s "$DONE_DIR/$base_name" 2>/dev/null)
            local new_size=$(stat -f%z "$output_file" 2>/dev/null || stat -c%s "$output_file" 2>/dev/null)
            local orig_mb=$(echo "scale=1; $orig_size / 1024 / 1024" | bc 2>/dev/null || echo "?")
            local new_mb=$(echo "scale=1; $new_size / 1024 / 1024" | bc 2>/dev/null || echo "?")
            local compression=$(echo "scale=1; (1 - $new_size / $orig_size) * 100" | bc 2>/dev/null || echo "?")
            log_success "Erfolgreich: $base_name (${duration}s) [${orig_mb}MB → ${new_mb}MB (${compression}% Ersparnis)]"
            return 0
        else
...
log_info "=== Video-Konvertierung gestartet ==="
...
# System und Encoder erkennen
SYSTEM=$(detect_system)
ENCODER=$(get_encoder "$SYSTEM")

# Dateien suchen
files=(*.mp4 *.wmv *.mov *.mkv *.avi *.webm)
file_count=${#files[@]}
